...
why(Drink):-
   known(ANSWER,BEGINN,ATTR,COND,VAL),
   (ANSWER==yes 
   -> with_output_to(string(Text1),format("Your Drink is ~w because:\n",[Drink])),
    with_output_to(string(Text2),format("it ~w ~w ~w ~w\n",[BEGINN,VAL,ATTR,COND])),
    new(Di,dialog('Why?')),
    send(Di, append,
    button(next, message(@prolog,why(Drink)))),
    send(Di, append,
    button(exit, message(Di,destroy))),
    send(Di,append,label(is,Text1)),
    send(Di,append,label(because,Text2)),
    send(Di,open)
    ;write("placeholder")
   ).
 ...
 * the answeres get getracted from the database. At the finish Dialog are 3 buttons for play again, show funfact and return to menue
 */
go:-
