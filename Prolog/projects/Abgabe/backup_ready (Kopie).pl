...
show_fact(Sentence):-
    ...
    send(Di, append, button(okay, message(@prolog, menue,Di))),
    send(Di,open).
...
go(D):-
    send(D,destroy),
    (drink(X)->with_output_to(string(Text),format("is your Drink ~w?",[X]))
    ;with_output_to(string(Text),format("Sorry no drink found :( try add your own in the menue!",[])),X=none
    ),
    ...
    send(Di, append,
        button(menue, message(@prolog,menue,Di))),
